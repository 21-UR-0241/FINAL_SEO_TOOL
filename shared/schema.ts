...
// Update your content table schema:
export const content = pgTable(
  "content",
  {
    ...
    // 🔥 CHANGE: Make websiteId NULLABLE
    websiteId: varchar("website_id")
      .references(() => websites.id, { onDelete: "cascade" }), // Removed .notNull()
    ...
    imageCount: integer("image_count").default(0), // 🔥 ADD: Track number of images
    imageCostCents: integer("image_cost_cents").default(0), // 🔥 ADD: Separate image cost tracking
...
export const imageMetadata = pgTable("image_metadata", {
  ...
  imageIdIdx: index("image_metadata_image_id_idx").on(table.imageId),
  websiteIdIdx: index("image_metadata_website_id_idx").on(table.websiteId),
  contentIdIdx: index("image_metadata_content_id_idx").on(table.contentId),
  altTextIdx: index("image_metadata_alt_text_idx").on(table.altText),
}));
...
export type InsertImageMetadata = typeof imageMetadata.$inferInsert;
