...
//     return fetchWithCredentials("/api/user/websites", {
//       method: "POST",
//       body: JSON.stringify(data),
//     }).then((res) => {
//       if (!res.ok) {
...
//     fetchWithCredentials(`/api/user/websites/${id}`, {
//       method: "PUT",
//       body: JSON.stringify(data),
//     }).then((res) => {
...
//     return fetchWithCredentials("/api/user/content/generate", {
//       method: "POST",
//       body: JSON.stringify(data),
//     }).then((res) => {
//       if (!res.ok) {
...
//     }
//   ) =>
//     fetchWithCredentials(`/api/user/content/${id}`, {
//       method: "PUT",
//       body: JSON.stringify(data),
//     }).then((res) => {
...
//     fetchWithCredentials(`/api/user/content/${id}/publish`, {
//       method: "POST",
//     }).then((res) => {
...
//   runSeoAnalysis: (websiteId: string, data?: { 
...
export const api = {
  ...
  // ============================================================================
  // BILLING & SUBSCRIPTION METHODS  ✅ MOVED INSIDE `api`
  // ============================================================================
  billing: {
    getPlans: () =>
      fetchWithCredentials("/api/billing/plans").then((res) => {
        if (!res.ok) throw new Error("Failed to fetch subscription plans");
        return res.json();
      }),

    getPlan: (planId: string) =>
      fetchWithCredentials(`/api/billing/plans/${planId}`).then((res) => {
        if (!res.ok) throw new Error("Plan not found");
        return res.json();
      }),

    getCurrentSubscription: () =>
      fetchWithCredentials("/api/billing/subscription").then((res) => {
    ...
    cancelSubscription: (immediate: boolean = false) => {
      ...
      return fetchWithCredentials("/api/billing/subscription/cancel", {
        method: "POST",
    ...
    reactivateSubscription: () => {
      ...
      return fetchWithCredentials("/api/billing/subscription/reactivate", {
        method: "POST",
      }).then((res) => {
    ...
    upgradeSubscription: (newPlanId: string) => {
      console.log("⬆️ Upgrading subscription to:", newPlanId);
      return fetchWithCredentials("/api/billing/subscription/upgrade", {
        method: "POST",
    ...
    validatePromoCode: (code: string, planId: string) => {
      console.log("🎟️ Validating promo code:", code);
      return fetchWithCredentials("/api/billing/promo/validate", {
        method: "POST",
        body: JSON.stringify({ code, planId }),
      }).then((res) => {
        if (!res.ok) {
          return res.json().then((error) => {
            throw new Error(error.error?.message || "Invalid promo code");
          });
        }
        return res.json();
      });
    ...
    getInvoices: (limit: number = 10) =>
      fetchWithCredentials(`/api/billing/invoices?limit=${limit}`).then(
        (res) => {
    ...
    getTransactions: (limit: number = 10) =>
      fetchWithCredentials(`/api/billing/transactions?limit=${limit}`).then(
        (res) => {
    ...
    getUsage: () =>
      fetchWithCredentials("/api/billing/usage").then((res) => {
