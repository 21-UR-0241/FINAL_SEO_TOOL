...
//       console.error("❌ Get content error:", response.status, text);
...
//     console.log('✅ Loaded content:', data.length, 'items');
//     return data;
//   },
...
//   async uploadImages(files: File[], websiteId: string, contentId?: string) {
...
//   async getUserImages(websiteId: string, contentId?: string) {
...
//     if (!response.ok) {
//       const text = await response.text();
//       console.error("Replace image error:", text);
...
//     if (!response.ok) {
//       const text = await response.text();
//       console.error("Delete content error:", text);
//       try {
//         const error = JSON.parse(text);
...
//   // Form state - REMOVED niche field
...
//     console.log('🌐 Website IDs:', websitesData.map(w => ({ id: w.id, name: w.name, type: typeof w.id })));
...
//   try {
//     console.log('🔍 DEBUG: Loading content for website:', {
//       selectedWebsite,
//       type: typeof selectedWebsite
//     });
    
//     // DON'T parse as integer - keep it as a string UUID
//     const data = await api.getWebsiteContent(selectedWebsite);
//     setContent(data);
//   } catch (error: any) {
...
//       errors.websiteId = "Please select a website";
//     }
...
//     const validTones = ["professional", "casual", "friendly", "authoritative", "technical", "warm"];
...
//   // 🔍 ADD DEBUG LOG #1 - Check state before sending
//   console.log('🔍 [Frontend] Form state before API call:', {
//     promptType: formData.promptType,
//     hasCustomPrompt: !!formData.customPrompt,
//     customPromptLength: formData.customPrompt?.length || 0,
//     customPromptPreview: formData.customPrompt?.substring(0, 100) + '...'
//   });
...
//     // 🔍 ADD DEBUG LOG #2 - Check data being sent to API
//     const requestData = {
...
//         requestData.customPrompt.substring(0, 100) + '...' : 
//         undefined
//     });

//     const result = await api.generateContent(requestData);
...
//       websiteId: selectedWebsite, // Keep as string UUID
//     });
...
// const downloadContentAsDoc = (item) => {
//   try {
//     const htmlContent = `
// <!DOCTYPE html>
...
//               {formData.customPrompt.length} characters
//             </p>
...
//                           brandVoice: e.target.value ? getWebsiteBrandVoice(e.target.value) : "",
//                         }))
//                       }
...
//                       Select a website to associate this content
//                     </p>
...
//                 : "Choose a website to view its content."}
//             </p>
//           </div>
//         </div>
//       )}
...
<button
  onClick={onClose}
  className="text-gray-400 hover:text-gray-500 p-2 -mr-2 touch-manipulation"
>
                <X className="w-6 h-6" />
              </button>
            </div>
...
{toast && (
  <div
    className={`fixed top-4 left-4 right-4 sm:left-auto sm:right-4 z-50 p-3 sm:p-4 rounded-lg shadow-lg border max-w-md ${
            toast.variant === "destructive"
              ? "bg-red-50 border-red-200 text-red-800"
              : toast.variant === "warning"
              ? "bg-yellow-50 border-yellow-200 text-yellow-800"
...
<button
  onClick={() => setToast(null)}
  className="ml-3 opacity-70 hover:opacity-100 flex-shrink-0 p-1 touch-manipulation"
>
              <X className="w-4 h-4" />
            </button>
          </div>
      ...
      {/* Page Header */}
        <div className="mb-8">
          <div className="flex-1 min-w-0 mb-4 sm:mb-0">
            <h2 className="text-xl sm:text-2xl font-bold leading-7 text-gray-900 sm:text-3xl">
              AI Content Generation
            </h2>
...
<button
  onClick={loadWebsites}
  disabled={isLoadingWebsites}
  className="inline-flex items-center justify-center w-full sm:w-auto px-3 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50"
>
            <RefreshCw
              className={`w-4 h-4 mr-2 ${
                isLoadingWebsites ? "animate-spin" : ""
...
<button
  onClick={() => setIsGenerateDialogOpen(true)}
  disabled={websites.length === 0}
  className="inline-flex items-center justify-center w-full sm:w-auto px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
>
            <Sparkles className="w-4 h-4 mr-2" />
            Generate Content
          </button>
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-end justify-center min-h-screen pt-4 px-2 sm:px-4 pb-20 text-center sm:block sm:p-0">
            <div
              ...
              onClick={() => !isGenerating && setIsGenerateDialogOpen(false)}
            ></div>
<div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-5xl w-full mx-4 sm:mx-0">
...
<button
  ...
  className={`p-3 sm:p-3 border-2 rounded-lg text-left transition-all touch-manipulation ${
    formData.aiProvider === "openai"
      ? "border-green-500 bg-green-50"
      : "border-gray-200 bg-white hover:border-gray-300"
  }`}
          ...
          <p className="text-xs text-green-800">
            <strong>System Prompt Benefits:</strong> Optimized for SEO, readability, and brand voice consistency. Includes automatic structure optimization and keyword placement.
                  ...
                  <div className="border-2 border-green-200 bg-green-50 rounded-lg p-4">
                    ...
                    <p className="text-xs text-gray-600 mb-4">
<span className="inline-flex items-center px-2.5 py-1 sm:px-2 sm:py-0.5 rounded text-xs font-medium bg-green-100 text-green-800 mr-1 whitespace-nowrap">
  <Sparkles className="w-3 h-3 mr-1" />
  Required
</span>
                      Select the language for your AI-generated content
...
<button
  type="button"
  onClick={() => setShowAdvanced(!showAdvanced)}
  className="flex items-center text-sm text-blue-600 hover:text-blue-500 touch-manipulation"
>
                        <Zap className="w-4 h-4 mr-1" />
                        Advanced Options
                      </button>
...
<button
  ...
  onClick={generateContent}
  disabled={isGenerating}
  className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 sm:py-2 bg-blue-600 text-sm sm:text-base font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:ml-3 sm:w-auto disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200"
>
                    {isGenerating ? (
                      <>
                        <Bot className="w-4 h-4 mr-2 animate-spin" />
...
<button
  ...
  disabled={isGenerating}
  className="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 sm:py-2 bg-white text-sm sm:text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto disabled:opacity-50 disabled:cursor-not-allowed"
>
                    {isGenerating ? "Please wait..." : "Cancel"}
                  </button>
                </div>
...
<button
  onClick={closeEditDialog}
  className="text-gray-400 hover:text-gray-500 focus:outline-none p-2 -mr-2 touch-manipulation"
>
                    <X className="w-6 h-6" />
                  </button>
                </div>
...
{/* Main Content - Side by Side Layout */}
<div className="flex flex-col lg:flex-row h-auto lg:h-[calc(100vh-200px)] max-h-[80vh]">
  {/* Left Side - WordPress Preview */}
  <div className="flex-1 p-4 sm:p-6 bg-gray-50 overflow-y-auto lg:border-r border-gray-200 max-h-96 lg:max-h-none">
    <div className="max-w-4xl mx-auto">
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 lg:p-8">
        <div className="mb-6 sm:mb-8">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 leading-tight mb-3 sm:mb-4">
            {editFormData.title || "Your Post Title"}
          </h1>
          <div className="flex flex-wrap items-center text-xs sm:text-sm text-gray-500 mb-3 sm:mb-4 gap-1">
            ...
            {editFormData.keywords && (
              <>
                ...
                <span className="w-full sm:w-auto mt-1 sm:mt-0">Keywords: {editFormData.keywords}</span>
              </>
            )}
          </div>
          {editFormData.excerpt && (
            <div className="bg-blue-50 border-l-4 border-blue-400 p-3 sm:p-4 mb-4 sm:mb-6">
              <p className="text-sm sm:text-base text-blue-800 italic">
                {editFormData.excerpt}
              </p>
            </div>
          )}
        ...
        {(editFormData.targetAudience ||
          ...
          <div className="mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-gray-200">
            ...
            <div className="flex flex-wrap gap-2">
              {editFormData.targetAudience && (
                <span className="inline-flex items-center px-3 py-1 sm:px-2.5 sm:py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 whitespace-nowrap">
                  Audience: {editFormData.targetAudience}
                </span>
              )}
              {editFormData.brandVoice && (
                <span className="inline-flex items-center px-3 py-1 sm:px-2.5 sm:py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 whitespace-nowrap">
                  Voice: {editFormData.brandVoice}
                </span>
              )}
              {editFormData.eatCompliance && (
                <span className="inline-flex items-center px-3 py-1 sm:px-2.5 sm:py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800 whitespace-nowrap">
                  <Shield className="w-3 h-3 mr-1" />
                  E-E-A-T Compliant
                </span>
              )}
              <span className="inline-flex items-center px-3 py-1 sm:px-2.5 sm:py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 whitespace-nowrap">
                Tone: {editFormData.tone}
              </span>
            </div>
          </div>
  ...
  {/* Right Side - Edit Form */}
  <div className="w-full lg:w-96 p-4 sm:p-6 bg-white overflow-y-auto">
    <div className="space-y-4 sm:space-y-6">
      {/* AI Provider Selection */}
      <div className="border border-purple-200 bg-purple-50 rounded-lg p-3 sm:p-4">
        ...
        <div className="space-y-2">
          <button
            ...
            className={`w-full p-3 border rounded-lg text-left text-xs sm:text-sm transition-all touch-manipulation ${
              editFormData.aiProvider === "openai"
                ? "border-green-500 bg-green-50"
                : "border-gray-200 bg-white hover:border-gray-300"
            }`}
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                <Cpu className="w-4 h-4 text-green-600 mr-2 flex-shrink-0" />
                <span className="font-medium">
                  OpenAI GPT-4
                </span>
              </div>
          ...
          >
            <div className="flex items-center">
              <Bot className="w-4 h-4 text-purple-600 mr-2 flex-shrink-0" />
              <span className="font-medium">
                Anthropic Claude
              </span>
            </div>
...
<button
  type="button"
  onClick={() => setShowImagePicker(true)}
  className="p-2 border border-indigo-300 rounded-lg hover:bg-indigo-100 touch-manipulation"
  title="Insert image at cursor"
>
              <div className="flex items-center justify-center">
                <ImagePlus className="w-4 h-4 text-indigo-600 sm:mr-0 mr-2" />
                <span className="sm:hidden text-xs text-indigo-700">Insert at Cursor</span>
              </div>
            </button>
          </div>
          {isUploadingImage && (
              ...
              {contentImages.map((img, idx) => (
                ...
                >
                  <img
                    src={img.url}
                    alt={img.altText}
                    className="w-12 h-10 sm:w-16 sm:h-12 object-cover rounded flex-shrink-0"
                    onError={(e) => {
                      e.target.src = "/placeholder-image.png";
                    }}
                  />
...
<button
  type="button"
  onClick={() => removeImage(img)}
  className="p-2 sm:p-1 hover:bg-red-100 rounded touch-manipulation"
  title="Remove image"
>
                    <X className="w-3 h-3 text-red-600" />
                  </button>
        ...
        <div className="space-y-3">
          <label className="flex items-start touch-manipulation">
            <input
              ...
              className="mt-1 rounded border-gray-300 text-orange-600 flex-shrink-0"
            />
            <div className="ml-3">
          ...
          {(editFormData.regenerateImages ||
            editFormData.includeImages) && (
            <div className="ml-6 pl-3 sm:pl-4 border-l-2 border-orange-200 space-y-3 bg-white p-3 rounded">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-2 py-1.5 sm:py-1 text-xs border border-gray-300 rounded-md"
                  >
                    <option value={1}>1 Image (Hero)</option>
                    <option value={2}>
                      2 Images (Hero + Support)
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="Enter the main content..."
          rows={12}
          className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 resize-none text-xs sm:text-sm font-mono"
          style={{ fontSize: "13px", lineHeight: "1.4" }}
        />
        {editFormErrors.body && (
          <p className="text-xs sm:text-sm text-red-600 mt-1">
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="Brief description or summary..."
          rows={3}
          className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 resize-none text-xs sm:text-sm"
        />
      </div>
...
<div className="bg-gray-50 px-4 sm:px-6 py-4 border-t border-gray-200">
  {/* Mobile text */}
  <div className="flex items-center text-xs sm:text-sm text-gray-500 mb-3 sm:mb-0 sm:hidden">
    <Edit className="w-4 h-4 mr-2 flex-shrink-0" />
    <span>
      Save or regenerate with AI
    </span>
  </div>
  ...
  <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 sm:justify-end">
    ...
    <button
      type="button"
      onClick={closeEditDialog}
      className="w-full sm:w-auto px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 order-3 sm:order-1"
    >
      Cancel
    </button>
    ...
    <button
      ...
      onClick={saveContent}
      disabled={isSaving}
      className="w-full sm:w-auto px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 disabled:opacity-50 disabled:cursor-not-allowed order-1 sm:order-2"
    >
      {isSaving ? (
        <div className="flex items-center justify-center">
          <Save className="w-4 h-4 mr-2 animate-pulse" />
    ...
    <button
      ...
      onClick={regenerateContent}
      disabled={isRegenerating}
      className="w-full sm:w-auto px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed order-2 sm:order-3"
    >
      {isRegenerating ? (
        <div className="flex items-center justify-center">
          <Brain className="w-4 h-4 mr-2 animate-spin" />
        ...
        <div className="flex items-center justify-center">
          ...
          {(editFormData.regenerateImages || editFormData.includeImages) && (
            <span className="ml-1 text-xs bg-orange-500 px-1.5 py-0.5 rounded whitespace-nowrap">
              +${(editFormData.imageCount * 0.04).toFixed(2)}
            </span>
          )}
        </div>
        ...
        {isLoadingWebsites ? (
          <div className="w-64 h-10 bg-gray-200 animate-pulse rounded-md"></div>
            ...
            <select
              value={selectedWebsite}
              onChange={(e) => setSelectedWebsite(e.target.value)}
              className="px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 w-full sm:w-64"
            >
            <option value="">Choose a website...</option>
            {websites.map((website) => (
              <option key={website.id} value={website.id}>
...
<button
  ...
  onClick={() => deleteContent(contentToDelete.id)}
  disabled={isDeleting}
  className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 sm:py-2 bg-red-600 text-sm sm:text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto disabled:opacity-50"
>
                  {isDeleting ? (
                    <>
                      <Loader2 className="w-4 h-4 mr-2 animate-spin" />
...
<button
  ...
  onClick={closeDeleteConfirm}
  disabled={isDeleting}
  className="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 sm:py-2 bg-white text-sm sm:text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto disabled:opacity-50"
>
                  Cancel
                </button>
              </div>
...
<button
  onClick={loadContent}
  disabled={isLoadingContent}
  className="inline-flex items-center justify-center w-full sm:w-auto px-3 py-2 sm:py-1.5 border border-gray-300 text-xs font-medium rounded text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50"
>
                <RefreshCw
                  className={`w-3 h-3 mr-1 ${
                    isLoadingContent ? "animate-spin" : ""
...
<button
  ...
  className={`${
    activeTab === "unpublished"
      ? "border-blue-500 text-blue-600"
      : "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300"
  } whitespace-nowrap py-4 px-2 sm:px-1 border-b-2 font-medium text-xs sm:text-sm transition-colors duration-200 touch-manipulation`}
>
                  Unpublished
<span
  className={`ml-2 ${
    activeTab === "unpublished"
      ? "bg-blue-100 text-blue-600"
      : "bg-gray-100 text-gray-900"
  } inline-flex items-center px-3 py-1 sm:px-2.5 sm:py-0.5 rounded-full text-xs font-medium whitespace-nowrap`}
>
  {unpublishedCount}
</span>
                </button>
...
>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-4 gap-3">
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap items-center gap-2 mb-3">
                            <span
                              className={`inline-flex items-center px-3 py-1 sm:px-2.5 sm:py-0.5 rounded-full text-xs font-medium border whitespace-nowrap ${getStatusColor(
                                item.status
                              )}`}
                            >
                              {getStatusIcon(item.status)}
                            ...
                            {item.aiProvider && (
                              <span className="inline-flex items-center px-2.5 py-1 sm:px-2 sm:py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-700 whitespace-nowrap">
                                {getProviderIcon(item.aiProvider)}
                                <span className="ml-1">
                                  {getProviderName(item.aiProvider)}
                                </span>
                            ...
                            {item.hasImages && item.imageCount > 0 && (
                              <span className="inline-flex items-center px-2.5 py-1 sm:px-2 sm:py-0.5 rounded text-xs font-medium bg-orange-100 text-orange-700 whitespace-nowrap">
                                <Image className="w-3 h-3 mr-1" />
                                {item.imageCount} image
                                {item.imageCount > 1 ? "s" : ""}
                              </span>
                        ...
                        <div className="flex flex-col sm:flex-row gap-2 sm:gap-2 sm:ml-4 ml-0 mt-3 sm:mt-0 w-full sm:w-auto">
                          {/* Download button */}
                          <button
                            onClick={() => downloadContentAsDoc(item)}
                            className="inline-flex items-center justify-center w-full sm:w-auto px-3 py-2 sm:py-1.5 border border-indigo-300 text-xs font-medium rounded text-indigo-700 bg-white hover:bg-indigo-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                            title="Download as Word document"
                          >
                            <Download className="w-3 h-3 mr-1" />
                            Download
                          ...
                          {activeTab === "unpublished" && (
                            <button
                              onClick={() => publishContent(item.id)}
                              disabled={isPublishing}
                              className={`inline-flex items-center justify-center w-full sm:w-auto px-3 py-2 sm:py-1.5 border border-transparent text-xs font-medium rounded text-white focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 ${
                                item.status === "publish_failed"
                                  ? "bg-orange-600 hover:bg-orange-700 focus:ring-orange-500"
                                  : "bg-green-600 hover:bg-green-700 focus:ring-green-500"
                              }`}
                          ...
                          {activeTab === "unpublished" && (
                            <button
                              onClick={() => openEditDialog(item)}
                              className="inline-flex items-center justify-center w-full sm:w-auto px-3 py-2 sm:py-1.5 border border-gray-300 text-xs font-medium rounded text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                            >
                              <Edit className="w-3 h-3 mr-1" />
                              Edit
                            </button>
                          ...
                          <button
                            ...
                            className="inline-flex items-center justify-center w-full sm:w-auto px-3 py-2 sm:py-1.5 border border-red-300 text-xs font-medium rounded text-red-700 bg-white hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 disabled:opacity-50"
                          >
                            {isDeleting && deletingContentId === item.id ? (
                              <>
                                <Loader2 className="w-3 h-3 mr-1 animate-spin" />
 ...
 <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between text-xs text-gray-500 border-t pt-3 gap-2">
  ...
  {item.eatCompliance && (
  <span className="inline-flex items-center px-3 py-1 sm:px-2 sm:py-0.5 rounded-full bg-purple-100 text-purple-800 text-xs whitespace-nowrap">
    <Shield className="w-3 h-3 mr-1" />
    E-E-A-T
  </span>
)}
