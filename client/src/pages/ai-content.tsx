...
const generateContent = async () => {
  ...
  try {
    ...
    console.log('🚀 Generating content with data:', {
      ...
      websiteIdType: typeof formData.websiteId,
      topic: formData.topic,
      aiProvider: formData.aiProvider
    });

    const result = await api.generateContent({
      // DON'T parse as integer - send as string UUID
      websiteId: formData.websiteId, // Remove any parseInt()
      topic: formData.topic,
      keywords: keywords,
      tone: formData.tone,
...
// // API utility functions - COMPLETE UPDATED VERSION
// // All functions now use ${API_URL} and include credentials
...
// const getNicheLabel = (nicheValue) => {
...
//     // Handle "all content" view
//     if (selectedWebsite === "__all__") {
...
//   // ============================================
//   // CRITICAL: Website OR Niche (mutually exclusive for generation)
//   // ============================================
//   if (!formData.websiteId && !formData.niche) {
//     errors.niche = "Please select a niche for standalone content OR select a website above";
//   }
  
//   // Prevent both being selected (should be impossible with UI, but validate anyway)
//   if (formData.websiteId && formData.niche) {
//     errors.niche = "Cannot select both website and niche - choose one or the other";
//   }
...
//   } else if (!validTones.includes(formData.tone)) {
//     errors.tone = "Invalid tone selected";
//   }

//   // ============================================
//   // Keywords Validation (Optional but recommended)
//   // ============================================
//   if (formData.keywords && formData.keywords.trim()) {
//     const keywordArray = formData.keywords.split(",").map(k => k.trim()).filter(k => k);
...
//     if (!formData.imageCount) {
//       errors.imageCount = "Please select number of images";
//     } else if (formData.imageCount < 1 || formData.imageCount > 3) {
//       errors.imageCount = "Image count must be between 1 and 3";
//     }
...
//       errors.imageStyle = "Please select an image style";
//     } else if (!validStyles.includes(formData.imageStyle)) {
//       errors.imageStyle = "Invalid image style selected";
//     }
//   }
...
//   // E-A-T Compliance recommendation for YMYL niches
//   const ymylNiches = ["peptides", "loans", "health", "finance"];
//   if (formData.niche && ymylNiches.includes(formData.niche) && !formData.eatCompliance) {
//     // This is a warning, not an error - don't block generation
//     console.warn(`⚠️ E-A-T compliance recommended for ${formData.niche} niche (YMYL content)`);
//   }

//   // Specific niche validations
//   if (formData.niche === "peptides") {
//     // Peptides should have scientific tone or professional
//     if (!["professional", "authoritative", "technical"].includes(formData.tone)) {
//       errors.tone = "Peptides content should use professional, authoritative, or technical tone";
//     }
//   }

//   if (formData.niche === "gambling") {
//     // Gambling should not use warm or overly casual tone
//     if (formData.tone === "warm") {
//       errors.tone = "Gambling content should use analytical or professional tone, not warm";
//     }
//   }
...
//   // Brand voice length check
//   if (formData.brandVoice && formData.brandVoice.length > 500) {
//     errors.brandVoice = "Brand voice description must be under 500 characters";
//   }

//   // Target audience length check
//   if (formData.targetAudience && formData.targetAudience.length > 300) {
//     errors.targetAudience = "Target audience description must be under 300 characters";
//   }
...
//         targetAudience: editFormData.targetAudience || undefined,
//         eatCompliance: editFormData.eatCompliance,
//         websiteId: selectedWebsite === "__all__" ? editingContent.websiteId : selectedWebsite,
//       });
...
// const downloadContentAsDoc = (item) => {
//   try {
//     // Create a simple HTML document with just title and content body
//     const htmlContent = `
// <!DOCTYPE html>
// <html xmlns:o='urn:schemas-microsoft-com:office:office' xmlns:w='urn:schemas-microsoft-com:office:word' xmlns='http://www.w3.org/TR/REC-html40'>
// <head>
...
//                               websiteId: e.target.value,
//                               brandVoice: e.target.value ? getWebsiteBrandVoice(e.target.value) : "",
//                               niche: e.target.value ? "" : prev.niche,
//                             }))
//                           }
//                           className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
//                         >
//                           <option value="">No website (standalone content)</option>
//                           {websites.map((website) => (
//                             <option key={website.id} value={website.id}>
//                               {website.name}
//                             </option>
...
//                         </select>
//                         <p className="text-xs text-gray-500 mt-1">
//                           Select a website to associate this content, or leave blank for standalone content
//                         </p>
//                         {formErrors.websiteId && (
//                           <p className="text-sm text-red-600 mt-1">
//                             {formErrors.websiteId}
...
//                       {/* Niche Selector - Only show when no website is selected */}
//                       {!formData.websiteId ? (
...
//                               Required for standalone content
//                             </span>
//                             The niche helps AI understand your content's industry and audience
//                           </p>
//                           {formErrors.niche && (
//                             <p className="text-sm text-red-600 mt-1">
//                               {formErrors.niche}
...
//                         </div>
//                       ) : (
//                         /* Content Tone - Show when website is selected */
//                         <div>
//                           <label className="block text-sm font-medium text-gray-700 mb-1">
//                             Content Tone
//                           </label>
...
//                     {/* Content Tone - Below grid when no website selected */}
//                     {!formData.websiteId && (
//                       <div>
//                         <label className="block text-sm font-medium text-gray-700 mb-1">
//                           Content Tone
...
//                             : "e.g., Latest WordPress Security Tips"
//                         }
//                         className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
//                       />
//                       {formErrors.topic && (
...
//                               placeholder={
//                                 formData.websiteId
//                                   ? "Leave empty to use website's brand voice"
//                                   : "Leave empty to use niche default"
//                               }
//                               className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
//                             />
//                           </div>
...
//             >
//               <option value="">Choose a website...</option>
//               <option value="__all__">All Content (including standalone)</option>
//               {websites.map((website) => (
//                 <option key={website.id} value={website.id}>
//                   {website.name} ({website.status})
//                 </option>
...
//                   </h3>
//                   <p className="text-sm text-gray-500">
//                     {selectedWebsite === "__all__"
//                       ? "All content including standalone pieces"
//                       : `Manage AI-generated content for ${getWebsiteName(selectedWebsite)}`}
//                   </p>
//                 </div>
//                 <button
...
//                               </span>
//                             </span>
//                             {/* Show niche badge for standalone content */}
//                             {item.niche && !item.websiteId && (
...
//                             {/* Show website badge if in "all content" view */}
//                             {selectedWebsite === "__all__" && item.websiteId && (
...
//                         </div>
//                         <div className="flex items-center space-x-2 ml-4">
//                           {/* Download button - only for standalone content */}
//                           {!item.websiteId && item.niche && (
//                             <button
//                               onClick={() => downloadContentAsDoc(item)}
//                               className="inline-flex items-center px-3 py-1.5 border border-indigo-300 text-xs font-medium rounded text-indigo-700 bg-white hover:bg-indigo-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
//                               title="Download as Word document"
...
//                           {/* Publish button - only for unpublished content with websiteId */}
//                           {activeTab === "unpublished" && item.websiteId && (
//                             <button
//                               onClick={() => publishContent(item.id)}
//                               disabled={isPublishing}
//                               className={`inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded text-white focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 ${
...
//                           {/* Note for standalone content */}
//                           {activeTab === "unpublished" && !item.websiteId && item.niche && (
...
//                 {isLoadingWebsites
//                   ? "Please wait while we fetch your websites from the database."
//                   : "Choose a website to view its content, or select 'All Content' to see everything including standalone pieces."}
//               </p>
//             </div>
//           </div>
//         )}
