...
declare global {
  interface Window {
    enhancedAiFixProgress?: {
      start: (websiteId: string, dryRun: boolean, options?: AIFixOptions) => Promise<any>;
      close: () => void;
    };
  }
}
...
export const EnhancedAIFixProgressDialog = () => {
  ...
  // Function to start the progress dialog with real backend polling
  const startProgressWithPolling = async (
    websiteId: string, 
    dryRun: boolean,
    ...
    const {
      ...
      enableEATImprovements = true,
      contentExpansionMinWords = 800,
      contentExpansionIdealWords = 1200,
      maxPagesPerSession = 5,
      enableReanalysis = true,
      forceReanalysis = false,
    } = options;

    // Initialize progress dialog
    const initialSteps = createInitialSteps(dryRun, enableContentExpansion, enableEATImprovements);
    setProgress({
      isOpen: true,
      currentStep: 0,
      totalSteps: initialSteps.length,
      ...
      overallStatus: "initializing",
      realTimeLogs: [
        `Starting AI fix analysis (dry run: ${dryRun})`,
        `Content Expansion: ${enableContentExpansion ? 'ENABLED ✅' : 'DISABLED ⚪'}`,
        `E-E-A-T Improvements: ${enableEATImprovements ? 'ENABLED ✅' : 'DISABLED ⚪'}`,
        enableContentExpansion ? `Target: ${contentExpansionMinWords}-${contentExpansionIdealWords} words` : '',
        `Max pages per session: ${maxPagesPerSession}`,
      ].filter(Boolean),
      currentOperation: "Initializing...",
      progressPercentage: 0,
    });
    ...
    // Start the actual backend process
    try {
      // Make the actual API call with all options
      const response = await fetch(`/api/user/websites/${websiteId}/ai-fix`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
      ...
      updateProgressError(error.message || 'Unknown error occurred');
      throw error;
    }
  };
  ...
  const updateProgressWithBackendLogs = (logs: string[], success: boolean) => {
    setProgress(prev => {
      ...
      logs.forEach((log, index) => {
        ...
        if (newSteps[stepIndex]) {
          if (log.includes('✅')) {
            ...
            newSteps[stepIndex].status = 'completed';
            newSteps[stepIndex].level = 'warning';
          } else if (log.includes('ℹ️') || log.includes('Starting') || log.includes('Processing')) {
            newSteps[stepIndex].status = 'running';
            newSteps[stepIndex].level = 'info';
          }
          ...
          if (cleanMessage.length > 0) {
            newSteps[stepIndex].message = cleanMessage;
          }
          newSteps[stepIndex].timestamp = timestampMatch ? timestampMatch[1] : undefined;
        }
      });
      ...
      return {
        ...
        progressPercentage,
        currentStep: completedSteps,
        overallStatus: success ? 'completed' : 'error',
        currentOperation: success ? 'Completed!' : 'Completed with warnings'
      };
    });
  };

  // Fallback update when no detailed logs available
  const updateProgressWithResult = (result: any) => {
    setProgress(prev => {
      const newSteps = [...prev.steps];
      ...
      newSteps.forEach(step => {
        step.status = 'completed';
        step.level = result.success ? 'success' : 'warning';
      });

      const logMessage = result.success 
        ? `✅ AI Fix completed: ${result.stats?.fixesSuccessful || 0} fixes applied`
        : `⚠️ AI Fix completed with issues`;

      return {
        ...prev,
        steps: newSteps,
        realTimeLogs: [...prev.realTimeLogs, logMessage],
  ...
  const updateProgressError = (errorMessage: string) => {
    setProgress(prev => {
      const newSteps = [...prev.steps];
      const currentStepIndex = newSteps.findIndex(s => s.status === 'running' || s.status === 'pending');
      
      if (currentStepIndex >= 0) {
        newSteps[currentStepIndex].status = 'error';
        newSteps[currentStepIndex].level = 'error';
      }
  ...
  const createInitialSteps = (
    ...
    // Add content expansion step if enabled
    if (contentExpansion) {
      baseSteps.push({
        id: "content_expansion",
        message: "Expanding thin content (AI-powered)",
        status: "pending",
        level: "info"
      });
    }

    // Add E-E-A-T step if enabled
    if (eatImprovements) {
      baseSteps.push({
        id: "eat_improvements",
        message: "Adding E-E-A-T signals (expertise, authority, trust)",
        status: "pending",
        level: "info"
      });
    }

    // Add standard fix steps
    baseSteps.push(
      {
        id: "meta_descriptions",
        message: "Processing meta description fixes",
  ...
  const getStepIcon = (stepId: string) => {
    const iconMap: Record<string, any> = {
      ...
      ai_recommendations: Brain,
      fixes_prioritization: Target,
      content_expansion: FileText,
      eat_improvements: Brain,
      meta_descriptions: FileText,
      heading_structure: FileText,
      alt_text: Image,
      completion: CheckCircle
  ...
  useEffect(() => {
    ...
    return () => {
      delete window.enhancedAiFixProgress;
    };
  }, []);
  ...
  return (
    <Dialog 
      open={progress.isOpen} 
      onOpenChange={(open) => {
        if (!open && (progress.overallStatus === "completed" || progress.overallStatus === "error")) {
          setProgress(prev => ({ ...prev, isOpen: false }));
        }
      }}
    >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <p className={`text-sm font-medium ${
                        step.status === "running" ? "text-blue-900" :
                        step.status === "completed" ? 
                          step.level === "success" ? "text-green-900" : 
                          step.level === "warning" ? "text-yellow-900" :
                          "text-gray-900" :
                        step.status === "error" ? "text-red-900" : "text-gray-600"
                      }`}>
                        {step.message}
            ...
            <div className="bg-black text-green-400 p-4 rounded-lg font-mono text-xs max-h-80 overflow-y-auto">
              {progress.realTimeLogs.length > 0 ? (
                progress.realTimeLogs.map((log, index) => (
                  ...
                  >
                    <span className="opacity-60">{String(index + 1).padStart(3, '0')}</span> {log}
                  </div>
                ))
              ) : (
                <div className="text-gray-500">Waiting for backend logs...</div>
