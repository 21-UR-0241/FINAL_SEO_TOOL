...
const proceedWithAuthentication = async (clientId, clientSecret) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
<div className="flex items-center justify-between h-auto sm:h-16 py-4 sm:py-0 flex-wrap gap-3">
  <div className="flex items-center">
...
{selectedAccount && (
  <div className="flex items-center space-x-2 px-2 sm:px-3 py-1 bg-gray-100 rounded-lg">
    ...
    <span className="text-xs sm:text-sm text-gray-700">
      {quotaUsage.used}/{quotaUsage.limit}
              ...
              <div className="relative">
<button
  className="flex items-center space-x-2 px-2 sm:px-4 py-2 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors flex-1 sm:flex-initial touch-manipulation"
  onClick={() => setAccountDropdownOpen(!accountDropdownOpen)}
>
  {selectedAccount ? (
    <>
      <User className="w-4 h-4" />
      <span className="text-xs sm:text-sm truncate max-w-[120px] sm:max-w-none">{selectedAccount.email}</span>
      <ChevronDown className="w-4 h-4 flex-shrink-0" />
    </>
  ) : (
    <>
      <User className="w-4 h-4" />
      <span className="text-xs sm:text-sm">Select Account</span>
      <ChevronDown className="w-4 h-4" />
    </>
  )}
</button>
...
<button
  onClick={handleAddAccount}
  disabled={isAuthenticating}
  className="px-3 sm:px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors flex items-center justify-center space-x-2 disabled:opacity-50 flex-1 sm:flex-initial whitespace-nowrap touch-manipulation"
>
  {isAuthenticating ? (
    <Loader2 className="w-4 h-4 animate-spin" />
  ) : (
  ...
  <span className="text-xs sm:text-sm">Add Account</span>
      ...
      {selectedAccount && (
        <div className="bg-white border-b border-gray-200">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
<div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:space-x-4 py-3">
  <label className="text-sm font-medium text-gray-700">
    Property:
  </label>
  <select
...
<button
  onClick={handleAddAccount}
  disabled={isAuthenticating}
  className="px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors inline-flex items-center justify-center space-x-2 disabled:opacity-50 w-full sm:w-auto touch-manipulation"
>
              {isAuthenticating ? (
                <>
                  <Loader2 className="w-5 h-5 animate-spin" />
...
<div className="flex space-x-1 border-b border-gray-200 mb-6 overflow-x-auto">
  {[
    ...
    <button
      key={tab.id}
      onClick={() => setActiveTab(tab.id)}
      className={`flex items-center space-x-2 px-3 sm:px-4 py-2 border-b-2 transition-colors whitespace-nowrap touch-manipulation ${
        activeTab === tab.id
          ? "border-blue-600 text-blue-600"
          : "border-transparent text-gray-600 hover:text-gray-900"
      }`}
...
<div className="flex flex-col sm:flex-row gap-2">
  ...
  <button
    onClick={handleIndexUrl}
    disabled={loading || !urlToIndex}
    className="w-full sm:w-auto px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors flex items-center justify-center space-x-2 disabled:opacity-50 whitespace-nowrap touch-manipulation"
  >
    {loading ? (
      <Loader2 className="w-4 h-4 animate-spin" />
    ) : (
...
<button
  onClick={handleBulkIndex}
  disabled={loading || !bulkUrls}
  className="w-full sm:w-auto px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors flex items-center justify-center space-x-2 disabled:opacity-50 touch-manipulation"
>
                {loading ? (
                  <Loader2 className="w-4 h-4 animate-spin" />
                ) : (
