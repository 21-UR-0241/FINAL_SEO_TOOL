...
/** Format timestamp label for mobile (shorter) */
const formatTimeLabelMobile = (dateStr: string | Date, tz = TZ) => {
  const date = new Date(dateStr);
  return new Intl.DateTimeFormat("en-US", {
    timeZone: tz,
    month: "short",
...
export default function PerformanceChart({ stats, isLoading, selectedWebsite }: PerformanceChartProps) {
  ...
  if (loading && !isFetching) {
    return (
      <Card>
        <CardHeader>
          <CardTitle className="text-base sm:text-lg">SEO Performance Trend</CardTitle>
        </CardHeader>
        <CardContent>
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-2 sm:p-3 border border-gray-200 rounded-lg shadow-lg max-w-xs">
          <p className="font-semibold text-xs sm:text-sm mb-2">{row?.label}</p>
            ...
            return (
              <div key={entry.name} className="mb-2 pb-2 border-b last:border-0">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span className="font-medium text-xs sm:text-sm truncate">{entry.name}</span>
                </div>
                <div className="ml-4 sm:ml-5 mt-1 space-y-0.5">
    ...
    <Card>
      <CardHeader>
        <div className="flex items-start sm:items-center justify-between gap-2">
          <CardTitle className="text-base sm:text-lg flex-1 min-w-0">
            <span className="hidden sm:inline">SEO Performance Trend</span>
            <span className="sm:hidden">Performance</span>
            {selectedWebsite && selectedWebsite !== "all" && chartData?.websites?.length === 1 && (
              <span className="block sm:inline text-xs sm:text-sm font-normal text-gray-600 sm:ml-2 mt-1 sm:mt-0 truncate">
          ...
          <button
            ...
            disabled={isFetching}
            className="p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg transition-colors disabled:opacity-50 flex-shrink-0 touch-manipulation"
          ...
          <div className="mt-3 space-y-2">
            <div className="flex flex-wrap gap-2 sm:gap-3 text-xs sm:text-sm">
              {chartData.websites
                ...
                .map((site: any) => (
                  <div key={site.id} className="flex items-center gap-1.5 sm:gap-2">
                    <div
                      className="w-2 h-2 sm:w-3 sm:h-3 rounded-full flex-shrink-0"
                      style={{ backgroundColor: site.color }}
                    />
                    <span className="text-gray-600 truncate max-w-[120px] sm:max-w-none">{site.name}</span>
      ...
      <CardContent className="px-2 sm:px-6">
        {chartData?.hasData ? (
            ...
            <ResponsiveContainer width="100%" height={250} className="sm:hidden">
              ...
              >
                ...
                <XAxis
                  ...
                  tickFormatter={(v) => formatTimeLabelMobile(new Date(v), TZ)}
                  tick={{ fontSize: 9 }}
                  tickMargin={5}
                  angle={-45}
                  textAnchor="end"
          ...
          <div className="h-[250px] sm:h-[350px] flex items-center justify-center px-4">
            <div className="text-center max-w-md">
              ...
              <h3 className="text-base sm:text-lg font-medium text-gray-900 mb-2">No SEO Reports Found</h3>
