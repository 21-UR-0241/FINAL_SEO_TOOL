...
import { db } from "./db";
import { lte, gte, count, eq, desc, lt, and, or, isNull, inArray,sql,not } from "drizzle-orm";
...
export interface IStorage {
  ...
  getUserDashboardStats(userId: string): Promise<{
    websiteCount: number;
    contentCount: number;
    standaloneContentCount: number; // 🔥 ADD THIS LINE
    avgSeoScore: number;
...
// 🔥 ADD THESE:
  getStandaloneContent(userId: string, options?: {
    niche?: string;
    limit?: number;
    offset?: number;
  }): Promise<Content[]>;
  
  getAllUserContent(userId: string, options?: {
    websiteId?: string;
    niche?: string;
    includeStandalone?: boolean;
    limit?: number;
  }): Promise<Array<Content & { websiteName?: string }>>;
  
  getContentStatsByNiche(userId: string): Promise<Array<{
    niche: string;
    count: number;
    avgSeoScore: number;
    totalCost: number;
...
async deleteWebsite(id: string): Promise<boolean> {
  ...
  if (websiteId === '__all__') {
    // Return ALL content (website + standalone)
    return await db
      .select()
      .from(content)
  ...
  // Get all content for user (both website and standalone)
  return await db
    .select()
    .from(content)
    .where(eq(content.userId, userId))
  ...
  async createContent(insertContent: InsertContent & { userId: string }): Promise<Content> {
    ...
    this.validateContentWebsiteOrNiche({
    websiteId: insertContent.websiteId,
    niche: insertContent.niche
  });
...
// ===============================
  ...
  const [usageRecord] = await db
    ...
    .values({
      websiteId: usage.websiteId || null,
      userId: usage.userId,
      model: usage.model,
...
async getUserDashboardStats(userId: string): Promise<{
  websiteCount: number;
  contentCount: number;
  standaloneContentCount: number; // 🔥 ADD THIS
  avgSeoScore: number;
  previousAvgSeoScore: number | null;
  recentActivity: number;
  scheduledPosts: number;
  ...
  const userContent = await this.getUserContent(userId);
    ...
    const standaloneContent = userContent.filter(c => !c.websiteId && c.niche);
  ...
  return {
    ...
    standaloneContentCount: standaloneContent.length, // 🔥 ADD THIS
...
async createScheduledTask(data: {
  ...
  // If using in-memory store (adjust for your actual DB)
...
async getScheduledTasks(websiteId: string, userId: string): Promise<ScheduledTask[]> {
  return (this.scheduledTasks || []).filter(
    task => task.websiteId === websiteId && 
            task.userId === userId &&
            task.status === 'pending'
  );
}
...
// ===============================
// STANDALONE CONTENT METHODS (ADD THESE)
// ===============================

/**
 * Get standalone content (niche-based, no website)
 */
async getStandaloneContent(userId: string, options?: {
  niche?: string;
  limit?: number;
  offset?: number;
}): Promise<Content[]> {
  let query = db
    ...
    .where(
      and(
        eq(content.userId, userId),
        isNull(content.websiteId) // Only content without a website
      )
    );
  
  // Filter by niche if specified
  if (options?.niche) {
    query = query.where(eq(content.niche, options.niche));
  }
...
/**
 * Get all content (both website-specific AND standalone)
 */
async getAllUserContent(userId: string, options?: {
  websiteId?: string;
  niche?: string;
  includeStandalone?: boolean;
  limit?: number;
  ...
  try {
    // Build base query with LEFT JOIN to websites
    let query = db
      .select({
        // Content fields
        id: content.id,
        ...
        // Website name (will be null for standalone content)
        websiteName: websites.name
      })
      .from(content)
      .leftJoin(websites, eq(content.websiteId, websites.id))
      .where(eq(content.userId, userId));
    
    // Filter by website if specified
    if (options?.websiteId) {
      query = query.where(eq(content.websiteId, options.websiteId));
    }
    
    // Filter by niche if specified (only for standalone content)
    if (options?.niche) {
      query = query.where(eq(content.niche, options.niche));
    }
    
    // Exclude standalone content if requested
    if (options?.includeStandalone === false) {
      query = query.where(isNotNull(content.websiteId));
    }
    ...
    console.error('Error getting all user content:', error);
    throw error;
  }
}

/**
 * Get content statistics by niche
 */
async getContentStatsByNiche(userId: string): Promise<Array<{
  niche: string;
  count: number;
  avgSeoScore: number;
  totalCost: number;
  ...
  try {
    const stats = await db
      .select({
        niche: content.niche,
        count: count(),
        avgSeoScore: sql<number>`COALESCE(AVG(${content.seoScore}), 0)`,
        totalCost: sql<number>`COALESCE(SUM(${content.costUsd}), 0)`
      })
      .from(content)
      .where(
        and(
          eq(content.userId, userId),
          isNotNull(content.niche),
          isNull(content.websiteId)
        )
      )
      .groupBy(content.niche);
    
    return stats.map(stat => ({
      niche: stat.niche || 'uncategorized',
      count: Number(stat.count) || 0,
      avgSeoScore: Math.round(Number(stat.avgSeoScore) || 0),
      totalCost: Number(stat.totalCost) || 0
    }));
  } catch (error) {
    console.error('Error getting content stats by niche:', error);
    return [];
  }
}

/**
 * Validate content has either websiteId OR niche (not both, not neither)
 */
private validateContentWebsiteOrNiche(data: {
  websiteId?: string | null;
  niche?: string | null;
}): void {
  const hasWebsite = !!data.websiteId;
  const hasNiche = !!data.niche;
  
  if (!hasWebsite && !hasNiche) {
    throw new Error('Content must have either websiteId (for website content) OR niche (for standalone content)');
  }
  
  if (hasWebsite && hasNiche) {
    throw new Error('Content cannot have both websiteId and niche - choose one');
  }
}
