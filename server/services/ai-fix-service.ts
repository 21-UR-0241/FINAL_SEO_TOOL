...
// private filterReadabilityExclusions(fixes: AIFix[]): {
//   toProcess: AIFix[];
//   skipped: number;
// } {
//   const toProcess: AIFix[] = [];
//   let skipped = 0;

//   for (const fix of fixes) {
//     if (fix.type !== 'poor_readability') {
//       toProcess.push(fix);
//       continue;
//     }

//     const pageUrl = fix.description.match(/https?:\/\/[^\s)]+/)?.[0] || '';
//     const urlLower = pageUrl.toLowerCase();
//     const excludePatterns = [
//       '/contact', '/checkout', '/cart', '/shop', '/product',
//       '/my-account', '/login', '/register', '/payment', '/order', '/wp-admin',
//     ];
...
//     if (shouldExclude) {
//       skipped++;
//       this.addLog(`ℹ️ Skipping readability for functional page: ${pageUrl}`, "info");
//     } else {
//       toProcess.push(fix);
//     }
//   }
...
//   if (skipped > 0) {
//     this.addLog(`ℹ️ Pre-filtered ${skipped} functional pages (not failures)`, "success");
//   }
...
//       if (readabilityScore >= 65) {
//         this.addLog(
//           `ℹ️ Readability already excellent for page ${fix.contentId} (${readabilityScore}/100 - ${gradeLevel})`,
//           "info"
//         );
//         return {
//           updated: false,
...
//           const wasReduction = improvedWordCount < currentWordCount - 20;
...
//         // ✅ CHECK READABILITY FIRST - Most important check
//         if (finalReadability <= readabilityScore) {
//           this.addLog(
//             `❌ Readability got worse or stayed same: ${readabilityScore} → ${finalReadability}`,
//             "error"
//           );
          
//           // Retry if readability dropped significantly (5+ points)
//           if (finalReadability < readabilityScore - 5) {
//             this.addLog(
//               `🔄 Retrying with readability emphasis (AI made it ${readabilityScore - finalReadability} points worse)...`,
//               "warning"
//             );
...
//                 contentHtml,
//                 retryContent,
//                 "Readability improvement (readability retry)",
//                 true
//               );
              
//               if (retryValidation.valid) {
//                 const retryFinalContent = this.ensureImagesPreserved(retryContent, originalImages);
//                 const retryText = this.extractTextFromHTML(retryFinalContent);
//                 const retryReadability = this.calculateReadabilityScore(retryText);
//                 const retryWordCount = retryText.split(/\s+/).filter(w => w.length > 0).length;
//                 const retryGrade = this.getGradeLevel(retryReadability);
                
//                 // Accept retry if it's better than original
//                 if (retryReadability > readabilityScore) {
//                   const improvement = retryReadability - readabilityScore;
//                   const meetsTarget = retryReadability >= 60;
//                   const targetStatus = meetsTarget 
//                     ? `✅ Target achieved (${retryGrade})` 
//                     : `⚠️ ${60 - retryReadability} points short of 60 target`;
                  
//                   this.addLog(
//                     `✅ Readability retry succeeded: ${readabilityScore} → ${retryReadability} (+${improvement} points)`,
//                     "success"
//                   );
                  
//                   return {
//                     updated: true,
//                     data: { content: retryFinalContent },
//                     description: `Improved readability from ${readabilityScore} to ${retryReadability} (+${improvement} points, ${retryGrade}) ${targetStatus} [readability retry succeeded]`,
//                   };
//                 } else {
//                   this.addLog(
//                     `❌ Readability retry still didn't improve: ${readabilityScore} → ${retryReadability}`,
//                     "error"
//                   );
//                 }
//               }
//             } catch (retryError: any) {
//               this.addLog(`Readability retry error: ${retryError.message}`, "error");
//             }
//           }
          
//           // If we get here, readability didn't improve (either no retry or retry failed)
//           return {
//             updated: false,
//             data: {},
//             description: `Readability worsened or stayed same (${readabilityScore} → ${finalReadability}) - REJECTED`,
//           };
//         }
        
//         // ✅ ONLY check word count if readability actually improved
//         if (finalWordCount < currentWordCount - 20) {
//           return {
//             updated: false,
//             data: {},
//             description: `Content reduced too much (${currentWordCount} → ${finalWordCount} words) despite readability improvement - REJECTED`,
//           };
//         }
        
//         // ✅ Success - readability improved AND word count acceptable
//         const improvement = finalReadability - readabilityScore;
//         const meetsTarget = finalReadability >= 60;
//         const targetStatus = meetsTarget 
//           ? `✅ Target achieved (${finalGrade})` 
...
//         this.addLog(
//           `✅ Readability improved: ${readabilityScore} → ${finalReadability} (+${improvement} points)`,
//           "success"
//         );
...
//   // ✅ SIMPLIFIED, READABILITY-FOCUSED PROMPT
//   const systemPrompt = `You are a readability expert. Your job is to make text easier to read while keeping the same information.

// READABILITY FORMULA (what actually improves scores):
// 1. **Short sentences** (10-20 words) - MOST IMPORTANT
// 2. **Simple words** (common, everyday vocabulary)
// 3. **Short paragraphs** (2-4 sentences max)
// 4. **Active voice** ("we did" not "it was done")

// CURRENT STATS:
// - Readability score: ${currentReadabilityScore}/100 (target: 60+)
// - Word count: ${currentWordCount} words
// - Acceptable range: ${currentWordCount - 20} to ${currentWordCount + 50} words

// YOUR PROCESS:
// 1. Find sentences over 25 words → split into 2-3 shorter sentences
// 2. Find complex words → replace with simpler synonyms
// 3. Find paragraphs over 4 sentences → split into 2 paragraphs
// 4. Find passive voice → change to active voice

// CRITICAL RULES:
// ✅ Keep the SAME information (don't remove facts)
// ✅ Preserve all __IMAGE_PLACEHOLDER_X__ markers exactly
// ✅ Return ONLY the HTML (no explanations)
// ❌ Don't add flowery adjectives or adverbs
// ❌ Don't combine sentences to save words
// ❌ Don't use complex synonyms (utilize→use, not utilize→leverage)

// ${isRetry ? `
// ⚠️ RETRY MODE: Previous attempt got readability WORSE.

// COMMON MISTAKES TO AVOID:
// - Adding adjectives/adverbs for word count ("very comprehensive detailed analysis")
// - Using formal vocabulary ("furthermore", "consequently", "nevertheless")
// - Creating compound sentences ("X, which Y, that Z...")
// - Passive voice ("it was determined" → use "we found")
...
// EXAMPLES:
// ❌ BAD (formal, long): "Furthermore, it is important to note that the comprehensive utilization of these sophisticated methodologies requires careful consideration of multiple factors."
// ✅ GOOD (simple, short): "Also, note that using these methods needs careful thought. You must consider many factors."

// ❌ BAD (compound): "The system, which was developed over several years, provides functionality that allows users to perform tasks more efficiently."
// ✅ GOOD (split): "The system was developed over several years. It helps users work more efficiently."
// ` : ''}`;
...
// Content (${currentWordCount} words, readability score: ${currentReadabilityScore}/100):
// ${contentForAI}

// Target: 60+ readability score

// ${isRetry ? `
// RETRY FOCUS: Previous attempt made readability WORSE. 
// Priority: SHORT SENTENCES with SIMPLE WORDS.

// Key actions:
// 1. Split EVERY sentence over 20 words
// 2. Replace ALL complex words with simple ones
// 3. Use active voice everywhere
// 4. Break paragraphs into 2-3 sentence chunks

// Examples of what to do:
// ❌ BAD: "The comprehensive implementation of the sophisticated system requires careful consideration."
// ✅ GOOD: "The system needs careful planning. We must implement it step by step."

// ❌ BAD: "Furthermore, it is important to note that the utilization of these methodologies..."
// ✅ GOOD: "Also, using these methods is important..."
// ` : ''}
...
//     // ✅ KEEP humanization but use gentle mode for readability content
//     cleaned = this.humanizeContent(cleaned, {
//       tone: 'professional',
//       aggressiveDeAI: false  // ✅ Don't be too aggressive - we want readability, not AI removal
//     });
...
//     const finalReadability = this.calculateReadabilityScore(
//       this.extractTextFromHTML(cleaned)
//     );
    
//     const wordDiff = finalWordCount - currentWordCount;
//     const readabilityDiff = finalReadability - currentReadabilityScore;
    
//     this.addLog(
//       `Readability AI ${isRetry ? '(retry)' : ''}:\n` +
//       `  Words: ${currentWordCount} → ${finalWordCount} (${wordDiff >= 0 ? '+' : ''}${wordDiff})\n` +
//       `  Score: ${currentReadabilityScore} → ${finalReadability} (${readabilityDiff >= 0 ? '+' : ''}${readabilityDiff})`,
//       finalReadability > currentReadabilityScore ? "success" : "warning"
//     );
...
// ✅ Updated fixWordPressContent (adds skipped/skipReason + readability allowShorter + captures public URL)
private async fixWordPressContent(
  creds: WordPressCredentials,
  fixes: AIFix[],
  fixProcessor: (
  ...
  const isReadabilityFixType = (type?: string) => {
    const t = (type || "").toLowerCase();
    return t === "poor_readability" || t === "low_readability";
  };
      ...
      for (const content of batch) {
        ...
        for (const fix of fixes) {
          try {
            ...
            // ✅ Skip safely (not failure; also should not count as "fixed")
            if (result.shouldSkip) {
              applied.push({
                ...fix,
                description: result.description,
                ...
                success: true,
                skipped: true,
                skipReason: result.skipReason || result.description,
                impact: fix.impact || "low",
              });
            ...
            if (result.updated) {
              if (result.data?.content) {
                // ✅ Important: allow shorter only for readability fixes
                const allowShorter = isReadabilityFixType(fix.type);
                ...
                if (!validation.valid) {
                  this.addLog(
                    `⚠️ ${validation.reason} - Skipping this fix`,
                    "warning"
                  );
                  applied[applied.length - 1].success = false;
                  applied[applied.length - 1].error = validation.reason;
        ...
        if (contentWasUpdated && Object.keys(updateData).length > 0) {
          try {
            const updated = await this.updateWordPressContent(
              creds,
              content.id,
              updateData,
              content.contentType
            ...
            const publicUrl = (updated as any)?.link || (content as any)?.link;
            if (publicUrl) {
              this.addLog(`Updated public URL: ${publicUrl}`, "info");
            } else {
              this.addLog(
                `Updated ${content.contentType} ${content.id} but no public URL (link) found`,
                "warning"
              );
            }
            ...
            errors.push(
              `WordPress update failed for ${content.id}: ${error.message}`
            );
            this.addLog(`WordPress update failed for ${content.id}`, "error");
          }
        }
...
// ✅ Updated fixWordPressContentWithPrecheck (adds skipped/skipReason in precheck + shouldSkip + readability allowShorter + captures public URL)
private async fixWordPressContentWithPrecheck(
  creds: WordPressCredentials,
  fixes: AIFix[],
  fixProcessor: (
  ...
  try {
    ...
    if (skippedContent.length > 0) {
      this.addLog(
        `✅ Skipped ${skippedContent.length} pages (already compliant):`,
        "success"
      );
      skippedContent.forEach(({ id, reason }) => {
        this.addLog(`   • Page ${id}: ${reason}`, "info");
    ...
    this.addLog(
      `📝 Processing ${contentNeedingFixes.length} pages that need fixes`,
      "info"
    );
    ...
    for (let i = 0; i < contentNeedingFixes.length; i += batchSize) {
      const batch = contentNeedingFixes.slice(
        i,
        Math.min(i + batchSize, contentNeedingFixes.length)
      );
    ...
    this.addLog(`   Total pages analyzed: ${allContent.length}`, "info");
    this.addLog(
      `   Already compliant (skipped): ${skippedContent.length}`,
      "success"
    );
    this.addLog(`   Processed (needed fixes): ${processedCount}`, "info");
    this.addLog(
      `   Successfully fixed: ${
        applied.filter((f) => f.success && !f.skipped && !f.description?.includes("already"))
          .length
      }`,
      "success"
    );
  ...
  return this.fixWordPressContent(
    ...
    async (content, fix) => {
      ...
      if (currentWordCount < 100) {
        ...
        updated: false,
        data: {},
        description: `Skipped readability: content too short (${currentWordCount} words)`,
        shouldSkip: true,
      ...
      if (readabilityScore >= 65) {
        ...
        updated: false,
        data: {},
        description: `Skipped readability: already excellent (${readabilityScore}/100 - ${gradeLevel})`,
        shouldSkip: true,
        skipReason: "already_excellent",
        };
      }
      ...
      try {
          ...
          if (wasReduction) {
            ...
            try {
              ...
              if (retryValidation.valid) {
                ...
                updated: false,
                data: {},
                description: `Skipped readability: no improvement after retry (${readabilityScore} → ${finalReadability})`,
                shouldSkip: true,
                skipReason: "no_improvement_after_retry",
                };
                }
                ...
                updated: false,
                data: {},
                description: `Skipped readability: ${retryValidation.reason}`,
                shouldSkip: true,
                skipReason: "retry_validation_failed",
                };
              }
            } catch (retryError: any) {
              this.addLog(`Retry attempt threw error: ${retryError.message}`, "error");
            ...
            updated: false,
            data: {},
            description: `Skipped readability: ${validation.reason}`,
            shouldSkip: true,
        ...
        updated: false,
        data: {},
        description: `Skipped readability: no improvement (${readabilityScore} → ${finalReadability})`,
        shouldSkip: true,
        skipReason: "no_improvement",
        };
        }
        ...
        updated: false,
        data: {},
        description: `Skipped readability: would reduce content too much (${currentWordCount} → ${finalWordCount} words)`,
        shouldSkip: true,
        skipReason: "content_reduction",
        };
        }
...
private async updateWordPressContent(
  ...
  data: any,
  contentType: "post" | "page" = "post"
): Promise<{ id: number; link?: string; [k: string]: any }> {
  const endpoint =
    contentType === "page"
      ? `${creds.url.replace(/\/$/, "")}/wp-json/wp/v2/pages/${id}`
      : `${creds.url.replace(/\/$/, "")}/wp-json/wp/v2/posts/${id}`;
  ...
  // ✅ updated.link is usually the public URL
  return updated;
}
