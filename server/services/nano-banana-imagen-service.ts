...
export class NanoBananaImagenService {
   ...
   * @param htmlContent The HTML content to insert images into
   * @param images Array of generated images
   * @param insertionStrategy Where to insert images: 'top', 'after-first-paragraph', or 'distributed'
   * @returns HTML content with images inserted
   */
  public insertImagesIntoContent(
    htmlContent: string,
    images: NanoBananaImagenImage[],
    insertionStrategy: 'top' | 'after-first-paragraph' | 'distributed' = 'after-first-paragraph'
  ): string {
    ...
    console.log(`📸 Inserting ${images.length} images using strategy: ${insertionStrategy}`);

    // Convert images to HTML
    const imageHtmlArray = images.map((img, index) => {
      return `<figure class="wp-block-image">
  <img src="data:${img.mimeType};base64,${img.base64}" alt="${img.altText}" class="wp-image-nanobana" />
  <figcaption>${img.altText}</figcaption>
    ...
    switch (insertionStrategy) {
      case 'top':
        // Insert all images at the top
        console.log('📍 Inserting images at top');
        return imageHtmlArray.join('\n\n') + '\n\n' + htmlContent;

      case 'after-first-paragraph':
        // Insert after the first paragraph (or heading)
        console.log('📍 Inserting images after first paragraph');
        const firstParagraphMatch = htmlContent.match(/(<p>.*?<\/p>|<h[1-6]>.*?<\/h[1-6]>)/s);
        if (firstParagraphMatch) {
          const firstParagraphEnd = htmlContent.indexOf(firstParagraphMatch[0]) + firstParagraphMatch[0].length;
          return (
            htmlContent.slice(0, firstParagraphEnd) +
            '\n\n' +
            imageHtmlArray.join('\n\n') +
            '\n\n' +
            htmlContent.slice(firstParagraphEnd)
          );
        }
        // If no paragraph found, insert at top
        console.log('⚠️ No paragraph found, inserting at top instead');
        return imageHtmlArray.join('\n\n') + '\n\n' + htmlContent;

      case 'distributed':
        // Distribute images throughout the content
        console.log('📍 Distributing images throughout content');
        const paragraphs = htmlContent.split(/(<\/p>|<\/h[1-6]>)/);
        if (paragraphs.length < 3) {
          // Not enough paragraphs, insert at top
          console.log('⚠️ Not enough paragraphs for distribution, inserting at top');
          return imageHtmlArray.join('\n\n') + '\n\n' + htmlContent;
        }
        ...
        if (images.length === 1) {
          insertionPoints.push(2); // After first paragraph
        } else if (images.length === 2) {
          insertionPoints.push(2, Math.floor(paragraphs.length / 2));
        } else {
          // Distribute evenly
          const step = Math.floor(paragraphs.length / (images.length + 1));
          for (let i = 1; i <= images.length; i++) {
            insertionPoints.push(step * i);
          }
        }

        console.log(`📊 Distribution points: ${insertionPoints.join(', ')}`);
        ...
        for (let i = 0; i < paragraphs.length; i++) {
          result += paragraphs[i];
          if (insertionPoints.includes(i) && imageIndex < images.length) {
            result += '\n\n' + imageHtmlArray[imageIndex] + '\n\n';
            console.log(`✅ Inserted image ${imageIndex + 1} at position ${i}`);
            imageIndex++;
          }
        }
      ...
      default:
        console.log('📍 Using default strategy (top)');
        return imageHtmlArray.join('\n\n') + '\n\n' + htmlContent;
    }
  }
