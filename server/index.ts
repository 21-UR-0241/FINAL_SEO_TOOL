...
app.use(helmet({
  ...
  contentSecurityPolicy: {
    directives: {
      defaultSrc: ["'self'"],
      connectSrc: ["'self'", "https://www.googleapis.com", "https://accounts.google.com", "https://oauth2.googleapis.com", "https://api.stripe.com"],
...
app.use((req: Request, _res: Response, next: NextFunction) => {
  if (req.session?.userId && req.session?.isAuthenticated) {
    ...
    req.user = {
      ...
      username: req.session.userId, // You may want to fetch actual username
...
(async () => {
  try {
    ...
    server.listen({ port, host }, () => {
      log(`🚀 Server running on http://${host}:${port}`);
      log(`💳 Billing API: http://${host}:${port}/api/billing/subscription`);
      log(`🔔 Stripe webhooks: http://${host}:${port}/api/billing/webhooks/stripe`);
      schedulerService.startScheduler(1);
      log('⏰ Content scheduler started');
      if (isDevelopment) log('🛠️ Development mode: Vite dev server enabled');
    });
